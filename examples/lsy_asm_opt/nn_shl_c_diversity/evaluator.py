...
candidate assembly program. This evaluator runs a command like::
  ...
  ./build/tsuperoptimizer evaluator /path/to/nn_shl_c.toml --rewrite=/abs/path/to/candidate.s --tc-json=/tmp/tc.json
...
LOGGER = logging.getLogger("examples.nn_shl_c.evaluator")
...
# In-memory baseline perf terms and on-disk persistence path.
BASE_PERF_TERMS = None
BASE_PERF_TERMS_FILE = THIS_DIR / "base_perf_terms.npy"

# Try to load persisted baseline on module import so it survives separate runs.
try:
    if BASE_PERF_TERMS_FILE.exists():
        BASE_PERF_TERMS = np.load(str(BASE_PERF_TERMS_FILE))
        LOGGER.info("Loaded BASE_PERF_TERMS from %s", BASE_PERF_TERMS_FILE)
except Exception:
    LOGGER.exception("Failed to load BASE_PERF_TERMS from disk")
...
def evaluate(program_path: str) -> EvaluationResult:
    ...
    try:
        ...
        return EvaluationResult(
            metrics={"combined_score": 0.0, "better_than_base_avg_position": 0.0},
...
def _evaluate(program_path: str) -> EvaluationResult:
    ...
    try:
        ...
        return EvaluationResult(metrics={"combined_score": 0.0, "better_than_base_avg_position": 0.0}, artifacts={"error": "compile_timeout"})
    except Exception as e:
        LOGGER.exception("Compile check failed")
        return EvaluationResult(metrics={"combined_score": 0.0, "better_than_base_avg_position": 0.0}, artifacts={"error": str(e)})
    ...
    if example_tc.exists():
        ...
        try:
            ...
            return EvaluationResult(metrics={"combined_score": 0.0, "better_than_base_avg_position": 0.0}, artifacts={"error": "timeout", "timeout": True})
    else:
    ...
    try:
        ...
        if m_terms:
            ...
            try:
                # 1. 先转成普通的 float 列表
                raw_list = [float(x) for x in re.split(r"\s*,\s*", perf_terms_str) if x != ""]
                
                # 2. 转换为 NumPy 数组 (double 精度)
                perf_terms = np.array(raw_list, dtype='float64')
                if BASE_PERF_TERMS is None:
                    BASE_PERF_TERMS = perf_terms.copy()
                    try:
                        np.save(str(BASE_PERF_TERMS_FILE), BASE_PERF_TERMS)
                        print("Set BASE_PERF_TERMS:", BASE_PERF_TERMS)
                        LOGGER.info("Saved BASE_PERF_TERMS to %s", BASE_PERF_TERMS_FILE)
                    except Exception:
                        LOGGER.exception("Failed to save BASE_PERF_TERMS to disk")
                
                print("perf_terms: ", perf_terms)
                
                # 3. 示例：如果要做减法 
                diff = perf_terms - BASE_PERF_TERMS
                negative_indices = np.where(diff < 0)[0]
                if negative_indices.size > 0:
                    negative_avg = negative_indices.mean()
                    print(f"Negative indices: {negative_indices}, average position: {negative_avg}")
                else:
                    negative_avg = 0.0
                # print("perf_terms - BASE_PERF_TERMS:", diff)
                
            except Exception:
                perf_terms = None
                negative_avg = 0
            
            if perf_terms is None:
    ...
    BASELINE_PERF = 55.55555555555556  # 你的初始程序 perf 值
    ...
    if negative_avg is None:
        negative_avg = 0
        
    print("better_than_base_avg_position",negative_avg)
    ...
    metrics = {
        ...
        "better_than_base_avg_position": negative_avg,
    }
