...
candidate assembly program. This evaluator runs a command like::
  ...
  ./build/tsuperoptimizer evaluator /path/to/__rshift_D2A.toml --rewrite=/abs/path/to/candidate.s --tc-json=/tmp/tc.json
...
LOGGER = logging.getLogger("examples.__rshift_D2A.evaluator")
...
def _extract_cex_from_output(text: str):
    ...
    if last is None:
        return []
    tail = text[last.end():]
    # capture up to end of line (or up to next [info])
    m2 = re.search(r"cex:\s*(.*?)(?:\n\s*\[info\]|$)", tail, re.I | re.S)
    if not m2:
        return []
    raw = m2.group(1).strip()
    if not raw:
        return []
    # If multiple examples are present, split by delimiters ; or \n
    parts = re.split(r"\s*[;\n]\s*", raw)
...
def _evaluate(program_path: str) -> EvaluationResult:
    ...
    toml_path = tsuper_root / "asm" / "final_benchmark" / "__rshift_D2A" / "__rshift_D2A.toml"
    ...
    # extract counterexamples from output (if any)
    ...
    BASELINE_PERF = 98.2258064516129  # 你的初始程序 perf 值
